...
public:
  ...
  mat4(f32 const* data) { mat4_ = XMLoadFloat4x4((XMFLOAT4X4*)data); }
  ...
  xvec3 rotation() const {
    auto quat_to_degrees = [](XMVECTOR const& quaternion) {
      ...
      f32 const pitch = std::atan2(rotationMatrix.r[1].m128_f32[2], rotationMatrix.r[2].m128_f32[2]);
      f32 const yaw   = std::atan2(
  ...
  void decompose(xvec3& translation, xvec3& scale, xvec3& rotation) const {
    ...
    // Extraer la escala
    XMVECTOR const scale_x = XMVector3Length(mat4_.r[0]);
    XMVECTOR const scale_y = XMVector3Length(mat4_.r[1]);
    XMVECTOR const scale_z = XMVector3Length(mat4_.r[2]);
  ...
  operator XMMATRIX() const { return mat4_; }

  xvec4 operator*(xvec3 const vec) const { return XMVector3Transform(vec, mat4_); }

  xvec4 operator*(xvec4 const vec) const { return XMVector4Transform(vec, mat4_); }

  mat4 operator*(mat4 const& mat) const { return XMMatrixMultiply(mat4_, mat); }
